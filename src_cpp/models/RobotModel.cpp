...
Controller::DesiredState<3> Robot::invKinematics(Path::DesiredPosition dpos,
  ...
  // compute manipulability from the desired Jacobian
  {
    auto JJT = J_des * J_des.transpose();
    manipulability = std::sqrt(JJT(0, 0) * JJT(1, 1) - JJT(0, 1) * JJT(1, 0));
  }
  ...
  /* Secondary tasks:
   * cast instructions into the null space of the redundant manipulator
   * for pinn training phase keep instructions light not sharp
   * use larger bounds on the prismatic and a damper on the
   * velocity of the revolute joints to prevent net revolution drift.
   */
  double Kh = 0.04;       // weight on prismatic joint centering bias
  double Kw = 0.01;       // damping term on carried over rotaional velocity
  double d = dqOld.dq[1]; // needed length for inverse kinematics

  // Dynamic damping on prismatic joint forces drastic response at limits and
  // almost no effect at standard operating ranges
  // double margin = 0.02; // prevents infinity at extremes
  // double dClamped =
  //     std::clamp(d, PRIS_LINK_MIN + margin, PRIS_LINK_MAX - margin);
  // double lengthRange = PRIS_LINK_MAX - PRIS_LINK_MIN;
  // double prisInstruction =
  //     -((dClamped - PRIS_LINK_MIN) - (PRIS_LINK_MAX - dClamped)) /
  //     (4.0 *
  //      std::pow((PRIS_LINK_MAX - dClamped) * (dClamped - PRIS_LINK_MIN), 2))
  //      *
  //     std::pow(lengthRange, 2);

  double prisCenter =
      (PRIS_LINK_MAX + PRIS_LINK_MIN) / 2; // center of desired length

  // penalizing undesired behavior for each joint space state
  // the faster the revolute joints spin and the further the prismatic joint is
  // from center the more they are penalized
  Eigen::Vector3d nullSpaceInstructions = {(1.0 - Kw) * dqOld.dqdot[0],
                                           -Kh * (d - prisCenter),
                                           (1.0 - Kw) * dqOld.dqdot[2]};

  // calculate the new desired state using the pesudo jac and num integration
  desState.dqdot =
      pseudoJ * (dpos.velocity.head<2>() + Ke * linearizationError.head<2>()) +
      (Eigen::Matrix3d::Identity() - pseudoJ * J_des) * nullSpaceInstructions;
  ...
  // clamp the prismatic joint to the joint limits without causing torque spikes
  prismaticLimitActive = false;
  if (desState.dq[1] > PRIS_LINK_MAX) {
    desState.dq[1] = PRIS_LINK_MAX;
    prismaticLimitActive = true;
    if (desState.dqdot[1] > 0.0)
      desState.dqdot[1] = 0.0; // don't kill motion away from limit
    if (desState.dqddot[1] > 0.0)
      desState.dqddot[1] = 0.0;
  } else if (desState.dq[1] < PRIS_LINK_MIN) {
    desState.dq[1] = PRIS_LINK_MIN;
    prismaticLimitActive = true;
    if (desState.dqdot[1] < 0.0)
      desState.dqdot[1] = 0.0;
    if (desState.dqddot[1] < 0.0)
      desState.dqddot[1] = 0.0;
  }
